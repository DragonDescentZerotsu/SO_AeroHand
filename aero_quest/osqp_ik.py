...
def _upper_triangle_csc(matrix: np.ndarray) -> sparse.csc_matrix:
    ...
    indices = np.concatenate(
        [np.arange(column + 1, dtype=np.int32) for column in range(n)]
    )
    indptr = np.zeros(n + 1, dtype=np.int32)
    indptr[1:] = np.cumsum(np.arange(1, n + 1, dtype=np.int32))
    data = np.concatenate(
        [matrix[: column + 1, column] for column in range(n)]
    )
...
def _upper_triangle_values(matrix: np.ndarray) -> np.ndarray:
    ...
    return np.concatenate(
        [matrix[: column + 1, column] for column in range(matrix.shape[0])]
    )
...
class OSQPVelocityIK:
    ...
    def __init__(
        ...
        self.joint_motion_weights = np.asarray(
            joint_motion_weights, dtype=np.float64
        ).reshape(self.joint_count)
        self.task_weights = np.asarray(
            task_weights, dtype=np.float64
        ).reshape(self.task_dimension)
    ...
    def _objective(
        ...
        regularization = np.diag(
            (effective_damping * self.joint_motion_weights) ** 2
        )
        ...
        if self.config.accel_weight > 0.0:
            hessian += self.config.accel_weight * np.eye(
                self.joint_count, dtype=np.float64
            )
    ...
    def _bounds(
        ...
        if max_accel > 0.0:
            lower = np.maximum(lower, self.prev_qdot - max_accel * dt)
            upper = np.minimum(upper, self.prev_qdot + max_accel * dt)
        if np.any(lower > upper):
            raise RuntimeError(
                f"Infeasible qdot bounds: lower={lower}, upper={upper}"
            )
        return lower, upper
    ...
    def solve(
        ...
        jacobian = np.asarray(jacobian, dtype=np.float64).reshape(
            self.task_dimension, self.joint_count
        )
        task_velocity = np.asarray(task_velocity, dtype=np.float64).reshape(
            self.task_dimension
        )
        q_current = np.asarray(q_current, dtype=np.float64).reshape(
            self.joint_count
        )
        joint_lower = np.asarray(joint_lower, dtype=np.float64).reshape(
            self.joint_count
        )
        joint_upper = np.asarray(joint_upper, dtype=np.float64).reshape(
            self.joint_count
        )

        hessian, gradient, damping, min_singular = self._objective(
            jacobian, task_velocity
        )
        lower, upper = self._bounds(
            q_current, joint_lower, joint_upper, dt
        )
        p_matrix = hessian + hessian.T
        ...
        if result.x is None or status.lower() not in {
            "solved",
            "solved inaccurate",
        }:
