...
- 默认运行 `full` 套件：16 个固定挑战、100 个固定随机种子的可达位姿，以及 16 秒连续 6D palm 轨迹。
...
完整模式的视频不会播放 100 个随机统计目标。它播放原有 5 点连续回归序列、移动到连续轨迹中心，以及完整 6D 周期轨迹，便于人工检查跟随平滑度；随机覆盖结果保存在 `summary.json`。
