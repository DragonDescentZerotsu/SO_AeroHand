...
blender --background --python scripts/debug/demo_blender_refraction_compare.py -- \
  --out-dir outputs/debug_rollouts/blender_liquid_refraction_compare \
  --engine all --cycles-samples 64
```

本地没有 `blender` 可执行文件但 Python 环境可 `import bpy` 时，`blender_render.py` 会自动 fallback 到当前 Python 解释器运行 `scripts/blender/render_trajectory_worker.py`，先渲 PNG 序列再用 `ffmpeg`/`imageio` 合成 mp4。若两者都不可用，入口仍会生成 `blender_render_manifest.json` 和 `render_command.sh` 供其他机器运行。使用 Blender binary 时，manifest 会把当前 `aero_sim` 的 site-packages 写入 `python_paths`，worker 启动后加入 `sys.path`，这样 binary Python 能 import `mujoco` 等 planner 依赖；不要把 MuJoCo 依赖重复手工装进 Blender 内置 Python。`render_trajectory_blender.py` 的默认 `--engine AUTO` 规则是：有 `--wet-state`/液体 overlay 时强制使用 Cycles；没有液体时使用 EEVEE 快速预览，效果可接受再决定是否升到 Cycles。正式 Blender 液体材质使用 Glass BSDF、IOR `1.333` 的透明折射路径；不要用 alpha/transmission 半透明贴片来判断清液折射效果。Blender 5.x API 注意事项：材质折射优先使用 `Material.use_raytrace_refraction`，`use_screen_refraction` 只作为旧版兼容；透明表面优先使用 `surface_render_method`，`blend_method` 只作为兼容字段；本机 Blender 5.0.1 的 `RenderSettings.engine` enum 可能只列出 `BLENDER_EEVEE`，但 `scene.render.engine = "CYCLES"` 在 enable cycles addon 后可正常工作，所以代码应检查实际 settable engine，不要只信 enum 列表。`blender_liquid.py` 的 tip 液体不要复用 MuJoCo 的分段 proxy；Blender 当前按 `tip_liquid_geometry` 和 `tip.volume_ul / tip.capacity_ul` 直接反推当前液面高度和半径，并为每帧生成一个闭合 tapered frustum mesh，默认最大半径小于 1mm，沿 `tip_axis_world` 放在 tip 内部。大容器液面高度以 `wet_state.jsonl` 中的 `surface.center_world` / `surface_center_world` 为准；Blender 只允许极小 render offset 避免透明面闪烁，当前为 `0.05mm`，不能用这个偏移改语义高度。Blender overlay 支持可选 `bulk_mesh_world`：有该字段时渲染液面以下真实填充 mesh，没有时退回只画液面薄片；MuJoCo 训练渲染仍只画大容器液面。正式或 demo 轨迹可以在 `wet_state.jsonl` 里写入 `tip_liquid_geometry` 覆盖默认值；`demo_blender_real_pipette_centrifuge_liquid.py` 会从真实 AutoBio `tip_200ul_vis/visual.obj` 估计可视内腔长度和顶部半径，并用 AutoBio meshplane `calculate_mesh(distance)` 写入离心管 bulk liquid，当前 tip 估计约为 `height_m=0.0325`、`top_radius_m=0.0010`。该 demo 使用真实 AutoBio pipette/tip mesh、真实 `centrifuge_1500ul_no_lid_vis` mesh 和 `MeshPlaneGeometry`，输出 `01_blender_real_pipette_centrifuge_full.mp4`、`02_blender_real_tip_close.mp4`、`03_blender_real_tube_liquid_close.mp4`、`wet_state.jsonl`、`real_pipette_centrifuge_liquid.npz` 和临时 MJCF；它的 qpos 是检查用可视化轨迹，不是正式专家轨迹。后续高保真液体可继续参考 AutoBio，把当前 demo 级 `bulk_mesh_world` JSON 升级为 `liquid.usd` animation。不要依赖 MuJoCo 的 `tip_liquid_seg_*` proxy，也不要让训练主线依赖 Blender。
...
```bash
conda run -n aero_sim python -m py_compile aero_tasks/blender_render.py aero_tasks/blender_scene.py aero_tasks/blender_liquid.py scripts/blender/render_trajectory_worker.py scripts/planning/render_trajectory_blender.py
conda run -n aero_sim pytest tests/test_blender_render.py tests/test_liquid.py
blender --background --python scripts/debug/demo_blender_refraction_compare.py -- --out-dir outputs/debug_rollouts/blender_liquid_refraction_compare_smoke --frames 2 --width 320 --height 180 --engine all --cycles-samples 8
conda run -n aero_sim python scripts/planning/render_trajectory_blender.py --trajectory outputs/piper_gripper_pipette_handoff/piper_gripper_pipette_handoff_expert.npz --wet-state outputs/debug_rollouts/blender_handoff_cycles_liquid_check/wet_state.jsonl --out-dir outputs/debug_rollouts/blender_handoff_cycles_liquid_check_smoke --output-name smoke_planner_cycles_liquid.mp4 --camera gripper_forward --width 320 --height 180 --stride 4093 --max-frames 2 --samples 8 --no-save-blend
```
